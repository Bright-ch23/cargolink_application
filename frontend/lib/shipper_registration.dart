  ...
  void _handleRegister() async {
    ...
    try {
      ...
      if (result.containsKey('error') && result['error'] == true) {
        ...
        final username = user['username'] ?? _nameController.text;
        final token = (result['access'] ?? '').toString();
        if (token.isEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Registration succeeded, but no access token was returned. Please log in.')),
          );
          Navigator.pushReplacementNamed(context, '/shipper_login');
          return;
        }
