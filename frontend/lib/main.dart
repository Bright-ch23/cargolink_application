  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      onGenerateRoute: (settings) {
        ...
        // Added Shipper Dashboard route as it was missing and required arguments
        ...
        // FIXED: Now correctly extracts arguments for the tracker
          case '/tracking':
            final args = settings.arguments as Map<String, dynamic>;
            return MaterialPageRoute(
              builder: (_) => TrackingMapScreen(
                bookingId: args['bookingId'].toString(), // Ensures it's a string
                token: args['token'],
              ),
            );
          ...
          case '/bidding_list':
            final args = settings.arguments as Map<String, dynamic>;
            return MaterialPageRoute(
              builder: (_) => CarrierBiddingScreen(
                token: args['token'],
                bookingId: args['bookingId'],
                loadTitle: args['loadTitle'],
              ),
            );
          case '/payment':
          ...
          case '/load_board':
            final args = settings.arguments as Map<String, dynamic>;
            return MaterialPageRoute(builder: (_) => LoadBoardScreen(token: args['token']));
          case '/load_details':
            final args = settings.arguments as Map<String, dynamic>;
            return MaterialPageRoute(
              builder: (_) => LoadDetailsScreen(
                token: args['token'],
                load: (args['load'] as Map).cast<String, dynamic>(),
              ),
            );
          case '/carrier_wallet':
